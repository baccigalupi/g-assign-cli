...
describe GAssign::API do
  let(:api) { GAssign::API.new(authenticator) }
  let(:authenticator) { double(username: 'username', password: 'password') }

  describe 'getting all assignments' do
    it "calls RestClient correctly" do
      expect(RestClient).to receive(:get).with(
        "http://username:password@g-assign-api.herokuapp.com/assignments"
      )
      api.all('assignments')
    end
  ...
  describe 'creating an assignment' do
    it "calls RestClient with the right url" do
      expect(RestClient).to receive(:post).with(
        "http://username:password@g-assign-api.herokuapp.com/assignments", anything, anything
      )
